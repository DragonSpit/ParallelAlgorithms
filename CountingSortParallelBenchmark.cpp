...
int CountingSortBenchmark(vector<unsigned>& uints)
{
	...
	for (int i = 0; i < iterationCount; ++i)
	{
		...
		vector<unsigned> sorted_reference(uints.size());
		for (size_t j = 0; j < uints.size(); j++)
		{
			//sorted_reference[j] = (unsigned char)uints[j];
			sorted_reference[j] = uints[j];
		}
		...
		print_results("std::sort of byte array", uintsCopy, uints.size(), startTimeRef, endTimeRef);
		...
		ParallelAlgorithms::counting_sort_v(uintsCopy, 0, uints.size());
		...
		print_results("Parallel Counting Sort", uintsCopy, uints.size(), startTime, endTime);
