...
int SumBenchmark(vector<unsigned long>& ulongs)
{
	...
	for (int i = 0; i < iterationCount; ++i)
	{
		...
		for (size_t i = 0; i < ulongs.size(); i++)
			sum_ref += u64Copy[i];
		//unsigned long long sum_ref = std::accumulate(u64Copy.begin(), u64Copy.end(), 0);	// TODO: Figure out why this function generates wrong answer
		...
		unsigned long long sum = ParallelAlgorithms::SumParallel(u64Array, 0, ulongs.size(), ulongs.size() / 12);
